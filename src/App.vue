...
export default {
  data() {
    return {
      players: playersData.map((player) => ({
        ...
        metaphoneName: this.encoder.doubleMetaphone(
          player.Name.trim().split('\n').find((line) => line.trim() !== '')
        )[0],
        metaphoneClub: this.encoder.doubleMetaphone(player.Verein)[0],
      })),
      learningMode: false,
  ...
  methods: {
    ...
    initializeSounds() {
      ...
      this.wrongSound = new Audio(require('@/assets/wrong.mp3'));
      this.pointSound = new Audio(require('@/assets/point.mp3'));
    },
    playSound(sound) {
      if (sound) {
        sound.play();
    ...
    findClosestMatch(spokenWord, isClub = false) {
      let closestMatch = null;
      let smallestDistance = Infinity;
      const metaphoneWord = this.encoder.doubleMetaphone(spokenWord)[0];

      this.players.forEach((player) => {
        const targetMetaphone = isClub ? player.metaphoneClub : player.metaphoneName;
        if (targetMetaphone) {
          const distance = levenshtein.get(metaphoneWord, targetMetaphone);
      ...
      recognizer.recognizeOnceAsync((result) => {
        ...
        if (result.reason === SpeechSDK.ResultReason.RecognizedSpeech) {
          ...
          if (this.learningMode) {
            const closestPlayer = this.findClosestMatch(spokenWord);
            ...
            const closestClub = this.findClosestMatch(spokenWord, true);
          ...
          console.error('Spracherkennung fehlgeschlagen:', result.reason);
